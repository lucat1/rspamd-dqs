...
local check_cw_dns = "._cw.{{ dqs_key }}.hbl.dq.spamhaus.net."
local check_file_dns = "._file.{{ dqs_key }}.hbl.dq.spamhaus.net."
			...
			if match then
				...
				local function dns_cb(_, _, results, err)
					...
					if string.find(tostring(results[1]), "127.0.") then
						rspamd_logger.infox(
							"found " .. cryptovalue .. " wallet %s (hashed: %s) in Cryptowallet blocklist",
							word,
							hash
						)
	...
	for _, p in ipairs(parts) do
		...
		local function dns_cb(_, _, results, err)
			...
			if string.find(tostring(results[1]), ret) then
				rspamd_logger.infox(
					"Attachment hash %s found in Spamhaus Malware HASHBL_" .. type,
					rspamd_hash.create_specific("sha256", ct):hex()
				)
